...
test('Z.ai Responses requests reserve output budget for the final planning message', async t => {
  const originalFetch = globalThis.fetch;
  let requestBody: Record<string, unknown> | undefined;
  globalThis.fetch = (async (_input: Parameters<typeof fetch>[0], init?: RequestInit) => {
    requestBody = JSON.parse(String(init?.body)) as Record<string, unknown>;
    return new Response(JSON.stringify({
      output: [{ type: 'message', content: [{ type: 'output_text', text: 'PI_OK' }] }],
    }), { status: 200, headers: { 'Content-Type': 'application/json' } });
  }) as typeof fetch;
  t.after(() => { globalThis.fetch = originalFetch; });
  const model = createPiModel({ ...configuration, baseUrl: 'https://api.z.ai/api/v1' });
  assert.equal(await model.reply({ message: '连接测试', history: [], memories: [], context: '{}' }), 'PI_OK');
  assert.equal(requestBody?.max_output_tokens, 1800);
  assert.deepEqual(requestBody?.reasoning, { effort: 'low' });
});
test('task planning is generated by model, persisted with sources, and never replaces old report on failure', async () => {
  let output = JSON.stringify(report);
  const f = fixture({ label: 'test-pi', reply: async input => {
    const context = JSON.parse(input.context) as { tasks?: Array<{ detail?: string }> };
    assert.match(input.context, /用户验收通过/);
    assert.match(input.message, /任务规划/);
    assert.match(input.message, /禁止.*继续优化.*处理问题/);
    assert.equal(context.tasks?.length, 1);
    assert.equal(context.tasks[0]?.detail, '清晰展示\n用户验收通过\n人工验收通过：已检查');
    assert.equal(Object.hasOwn(context.tasks[0] ?? {}, 'objective'), false);
    return output;
  } });
  ...
  output = `{"summary":"基于已同步记录：已验收 1 项。","completed":[{"text":"手机列表已人工验收。","taskIds":["T-1"]`;
  const recovered = await f.service.generateReport('2026-09-20');
  assert.match(recovered.content.summary, /已验收 1 项/);
  assert.deepEqual(recovered.content.completed[0]?.taskIds, ['T-1']);
  output = '{"summary":"结构不完整"}';
  await assert.rejects(f.service.generateReport('2026-09-20'), /结构不完整/);
  assert.equal(f.service.reports('2026-09-20').length, 2);
  ...
  assert.equal(f.service.reports('2026-09-20')[0]?.id, recovered.id);
  assert.ok(f.service.reports('2026-09-20').some(item => item.id === saved.id));
});
